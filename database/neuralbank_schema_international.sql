...
CREATE INDEX idx_cliente_metadata ON core.cliente USING GIN(metadata);
...
CREATE INDEX idx_cuenta_metadata ON core.cuenta USING GIN(metadata);
...
CREATE INDEX idx_credito_metadata ON core.credito USING GIN(metadata);
...
CREATE INDEX idx_cuota_mora ON core.cuota(dias_mora) WHERE dias_mora > 0;
...
CREATE INDEX idx_transaccion_metadata ON core.transaccion USING GIN(metadata);
